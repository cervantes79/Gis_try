...
conn=None
def get_query(query):
    con = psycopg2.connect(user=db_user, password=db_password, host=db_host, port=db_port,database=db_database)
    rlist = []
    try:
        if (con):
            curr = con.cursor()
...
def load_csv():
    ...
    for i in rows2:
        ...
        address3=  str(i[9])
        address4= str(i[10])
        query="insert into public.ukdata (rowid, country_code, address1, address2, postcode, geom,  number_value, address3, address4)"\
        " values (%s,%s,%s,%s,%s, {},%s,%s,%s)".format(geom)
        set_query(query=query,data_list=[rowid, country_code, address1, address2, postcode, number_value, address3, address4])
    close_conn()
...
def check_db():
    ...
    if ls[0]["count"]==0:
        query="CREATE DATABASE test WITH OWNER = postgres ENCODING = 'UTF8' LC_COLLATE = 'en_US.utf8' LC_CTYPE = 'en_US.utf8' TABLESPACE = pg_default CONNECTION LIMIT = -1; CREATE EXTENSION postgis; "
        set_query2(query)
    query="SELECT count(*) FROM pg_catalog.pg_tables WHERE schemaname != 'pg_catalog' AND schemaname != 'information_schema' and tablename='ukdata';"
    ls=get_query(query=query)
    if ls[0]["count"]==0:
